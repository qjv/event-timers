...
impl NotificationState {
    ...
    pub fn mark_notified(&mut self, event_id: &TrackedEventId, event_start_time: i64, minutes_before: u32) {
    ...
    pub fn was_notified(&self, event_id: &TrackedEventId, event_start_time: i64, minutes_before: u32) -> bool {
    ...
    /// Clean up old notified entries (events that have passed)
    pub fn cleanup_old_notifications(&mut self, current_time: i64) {
        self.notified_reminders.retain(|key| {
            // Keep entries for events that haven't started yet or started recently
            key.event_start_time > current_time - 3600 // Keep for 1 hour after start
        });
    }
